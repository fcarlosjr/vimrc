    ...
    for arg in arglist
        ...
        if is_current_buffer
            ...
            if are_all_windows_showing_current_buffer
                let listed_buffers = filter(range(1, bufnr('$')),
                      \ {_, b -> buflisted(b) && getbufvar(b, '&buftype') ==# ''})

                if len(listed_buffers) == 1 && !getbufvar(bufnr('%'), '&modified')
                    silent! execute 'enew | bdelete'.a:bang.' # | redraw'
                elseif len(listed_buffers) == 1
                    execute 'enew | bdelete'.a:bang.' # | redraw'
                else
                    execute 'bnext | bdelete'.a:bang.' # | redraw'
                endif
